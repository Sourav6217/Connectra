...
with tabs[0]:
    ...
    with col_l:
        ...
        <div class='g-card anim-up'>
          <div style='display:flex;align-items:center;gap:16px;margin-bottom:18px;'>
            ...
            <div style='flex:1;'>
              ...
              <div style='margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;'>
                ...
                {"<span class='tag tp'>NFT Verified ⬡</span>" if row.get("nft_token_id") else ""}
          ...
          <div class='bar-bg'><div class='bar' style='--w:{talent_score}%;width:{talent_score}%;'></div></div>
          <hr class='div'>
    ...
    with col_r:
        ...
        if apps_df.empty:
            st.markdown("""
            <div style='text-align:center;padding:24px;color:#2a4a34;font-size:13px;'>
              No applications yet.<br>Browse the Marketplace!
            </div>
            """, unsafe_allow_html=True)
        else:
            STATUS_COLORS = {
                "Pending": "ta", "Shortlisted": "tb",
                "Hired": "tg", "Rejected": "tr"
            }
            for _, app in apps_df.head(5).iterrows():
                ...
                <div style='padding:10px 0;border-bottom:1px solid rgba(29,158,117,.09);'>
                  <div style='display:flex;justify-content:space-between;align-items:center;'>
                    <div>
                      <div style='font-size:13px;font-weight:600;color:#fff;'>{app.get("title","Job")[:30]}</div>
        ...
        <div class='g-card anim-up d3' style='margin-top:14px;'>
          ...
          {html_bar("Completion Rate", float(row["completion_rate"]),
                    "bar" if row["completion_rate"]>=80 else "bar-a")}
          {html_bar("Project Load", min(float(row["projects"])/25*100,100), "bar-b")}
        </div>
        """, unsafe_allow_html=True)
